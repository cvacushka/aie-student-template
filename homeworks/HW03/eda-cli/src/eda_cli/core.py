...
def compute_quality_flags(
    ...
    # Новые эвристики требуют доступ к исходному DataFrame
    has_constant_columns = False
    has_high_cardinality_categoricals = False
    has_many_zero_values = False
    constant_column_names: List[str] = []
    high_cardinality_column_names: List[str] = []
    zero_rich_column_names: List[str] = []
    ...
    if has_high_cardinality_categoricals:
        score -= 0.05  # высокая кардинальность может быть проблемой
    if has_many_zero_values:
        score -= 0.05  # много нулей может указывать на проблемы
