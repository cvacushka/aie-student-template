...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags['has_constant_columns']:
            f.write(f"  - Константные колонки: {', '.join(quality_flags['constant_column_names'])}\n")
        f.write(f"- Высокая кардинальность категориальных признаков (> {quality_flags['high_cardinality_threshold']}): **{quality_flags['has_high_cardinality_categoricals']}**\n")
        if quality_flags['has_high_cardinality_categoricals']:
            f.write(f"  - Колонки с высокой кардинальностью: {', '.join(quality_flags['high_cardinality_column_names'])}\n")
        f.write(f"- Много нулевых значений (доля > {quality_flags['zero_share_threshold']:.0%}): **{quality_flags['has_many_zero_values']}**\n")
        if quality_flags['has_many_zero_values']:
            f.write(f"  - Колонки с большим количеством нулей: {', '.join(quality_flags['zero_rich_column_names'])}\n")
        f.write(f"\n**Параметры анализа:**\n")
        ...
        if missing_df.empty:
            ...
            # Список проблемных колонок по пропускам
            ...
            if not problematic_cols.empty:
                ...
                for col_name, row in problematic_cols.iterrows():
                    f.write(f"- `{col_name}`: {row['missing_share']:.2%} ({int(row['missing_count'])} пропусков)\n")
                f.write("\n")
