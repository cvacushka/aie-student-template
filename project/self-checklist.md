...
1. `python -m pytest` — тесты подтверждают генерацию данных, обучение модели и работу `/predict`.
2. `python -m airguard.models.train` — модель обучается воспроизводимо и обновляет `models/metrics.json`.
3. `/health` — сервис видит загруженный artifact модели.
4. `/predict` — ответ строится реальной моделью из `models/airguard_model.joblib`.
5. `/metrics` — после запросов появляются счётчики и средняя задержка.
